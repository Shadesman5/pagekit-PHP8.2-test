...
class ImageHelper extends Helper {
	...
	protected function validResource ($resource) {
		...
		if (!$resource['src'] || !App::locator()->get($resource['src'])) {
			...
			$resource['src'] = 'packages/bixie/pk-framework/assets/noimage.jpg';
	...
	protected function getCachePath ($source, $options) {
		...
		$folder = $this->app->module('bixie/pk-framework')->getImageCachepath();
	...
	public static function clearCache ($options = []) {
		...
		if (@$options['temp'] and $cache_path = App::module('bixie/pk-framework')->getImageCachepath()) {
